...
TEST(BVHNode, LeafNodeCanStorePrimitiveRange) {
    ...
    node.bounds  = AABB(Float3(0.0f, 0.0f, 0.0f), Float3(1.0f, 2.0f, 3.0f));
    node.start   = 4;
    node.count   = 7;
    node.is_leaf = true;
    ...
    expect_vec_near(node.bounds.lower_corner, Float3(0.0f, 0.0f, 0.0f));
    expect_vec_near(node.bounds.upper_corner, Float3(1.0f, 2.0f, 3.0f));
}

TEST(BVHNode, InternalNodeCanStoreChildLinks) {
    BVHNode node;

    node.bounds  = AABB(Float3(-1.0f, -2.0f, -3.0f), Float3(3.0f, 2.0f, 1.0f));
    node.left    = 1;
    node.right   = 2;
    node.start   = -1;
    node.count   = 0;
    ...
    EXPECT_EQ(node.count, 0);
    EXPECT_TRUE(node.bounds.is_valid());
    expect_vec_near(node.bounds.center(), Float3(1.0f, 0.0f, -1.0f));
}
