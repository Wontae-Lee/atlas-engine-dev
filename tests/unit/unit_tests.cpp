...
TEST(Unit, BuilderConstructsDynamicUnitAndCanonicalizesKinematics) {
    ...
    expect_vec_near(*unit.acceleration(), Float3(0.0f, 0.0f, 0.0f));
}
...
TEST(Unit, UpdateAppliesVelocity) {
    ...
    expect_vec_near(unit.sync().translation, Float3(1.0f, 0.0f, 0.0f));
}
...
TEST(Unit, GeometryOperatorRemainsValidAfterGeometryOwnerDies) {
    ...
    EXPECT_TRUE(op.is_inside(Float3(0.0f, 0.0f, 0.0f)));
    expect_vec_near(op.centroid(), Float3(0.0f, 0.0f, 0.0f));
}
...
TEST(Unit, MoveAndRotateUpdatePose) {
    ...
    unit.move(Float3(1.0f, 2.0f, 3.0f));
    unit.rotate(Float3(0.0f, 0.0f, 1.0f), pi / 2.0f);

    expect_vec_near(unit.sync().translation, Float3(1.0f, 2.0f, 3.0f));
    expect_vec_near(unit.sync().sync_dir_to_world(Float3(1.0f, 0.0f, 0.0f)), Float3(0.0f, 1.0f, 0.0f));
}
