...
TEST(Vector3, DefaultConstructible) {
    const atlas::math::Vector<float, 3> v {};
    (void)v;
    SUCCEED();
}

TEST(Vector3, UniformScalarConstructor) {
    const atlas::math::Vector<float, 3> v(2.5f);
...
TEST(Vector3, InitializerListConstructor) {
    const atlas::math::Vector<float, 3> v { 1.0f, 2.0f, 3.0f };
...
TEST(Vector3, DataAndIndexAccess) {
    atlas::math::Vector<float, 3> v(1.0f, 2.0f, 3.0f);
...
TEST(Vector3, VectorOpsInPlace) {
    atlas::math::Vector<float, 3> a(1.0f, 2.0f, 3.0f);
    const atlas::math::Vector<float, 3> b(10.0f, 20.0f, 30.0f);
...
TEST(Vector3, MinMax) {
    const atlas::math::Vector<float, 3> v(-1.0f, 3.0f, 2.0f);
    EXPECT_FLOAT_EQ(v.min(), -1.0f);
    EXPECT_FLOAT_EQ(v.max(), 3.0f);
}

TEST(Vector3, DotAndCross) {
    const atlas::math::Vector<double, 3> a(1.0, 2.0, 3.0);
    const atlas::math::Vector<double, 3> b(4.0, 5.0, 6.0);

    const double d = a.dot(b);
    EXPECT_DOUBLE_EQ(d, 32.0);

    const atlas::math::Vector<double, 3> c = a.cross(b);
    EXPECT_DOUBLE_EQ(c.x, -3.0);
    EXPECT_DOUBLE_EQ(c.y, 6.0);
    EXPECT_DOUBLE_EQ(c.z, -3.0);

    const atlas::math::Vector<double, 3> c2 = atlas::math::cross(a, b);
...
TEST(Vector3, LengthAndNormalize) {
    ...
    atlas::math::Vector<double, 3> v(3.0, 4.0, 12.0);
    ...
    const atlas::math::Vector<double, 3> n = v.normalized();
...
TEST(Vector3, MajorMinorAxisDefinition) {
    const atlas::math::Vector<float, 3> v(1.0f, -5.0f, 3.0f);
...
TEST(Vector3, ReflectedAndProjected) {
    ...
    const atlas::math::Vector<double, 3> r = v.reflected(n);
    EXPECT_TRUE(atlas::test::vec_near(r, atlas::math::Vector<double, 3>(1.0, 2.0, 3.0), eps));

    const atlas::math::Vector<double, 3> nn = n.normalized();

    const atlas::math::Vector<double, 3> p = v.projected(nn);

    const atlas::math::Vector<double, 3> proj_dir   = nn * v.dot(nn);
    const atlas::math::Vector<double, 3> proj_plane = v - proj_dir;

    const bool ok = atlas::test::vec_near(p, proj_dir, eps) || atlas::test::vec_near(p, proj_plane, eps);
    EXPECT_TRUE(ok);

    const atlas::math::Vector<double, 3> r2 = atlas::math::reflected(v, nn);
    EXPECT_TRUE(atlas::test::vec_near(r2, atlas::math::Vector<double, 3>(1.0, 2.0, 3.0), eps));

    const atlas::math::Vector<double, 3> p2 = atlas::math::projected(v, nn);
...
TEST(Vector3, TangentialBasisOrthogonality) {
    ...
    const auto [u0, u1] = atlas::math::tangential(n);
    EXPECT_TRUE(std::abs(n.dot(u0)) < eps);
    EXPECT_TRUE(std::abs(n.dot(u1)) < eps);
}

TEST(Vector3, CastTo) {
    const atlas::math::Vector<float, 3> vf(1.25f, -2.5f, 3.75f);
    const atlas::math::Vector<double, 3> vd = vf.cast_to<double>();

    EXPECT_DOUBLE_EQ(vd.x, 1.25);
    EXPECT_DOUBLE_EQ(vd.y, -2.5);
    EXPECT_DOUBLE_EQ(vd.z, 3.75);

    const auto vi = atlas::math::cast_to<int>(vf);
    EXPECT_EQ(vi.x, 1);
    EXPECT_EQ(vi.y, -2);
    EXPECT_EQ(vi.z, 3);
}
...
TEST(Vector3, FreeOperatorsBasic) {
    ...
    const atlas::math::Vector<float, 3> a(1.0f, 2.0f, 3.0f);
    const atlas::math::Vector<float, 3> b(10.0f, 20.0f, 30.0f);

    const atlas::math::Vector<float, 3> s1 = a + b;
    EXPECT_TRUE(atlas::test::vec_near(s1, atlas::math::Vector<float, 3>(11.0f, 22.0f, 33.0f), eps));

    const atlas::math::Vector<float, 3> s2 = a - b;
    EXPECT_TRUE(atlas::test::vec_near(s2, atlas::math::Vector<float, 3>(-9.0f, -18.0f, -27.0f), eps));

    const atlas::math::Vector<float, 3> s3 = a * 2.0f;
    EXPECT_TRUE(atlas::test::vec_near(s3, atlas::math::Vector<float, 3>(2.0f, 4.0f, 6.0f), eps));

    const atlas::math::Vector<float, 3> s4 = 2.0f * a;
    EXPECT_TRUE(atlas::test::vec_near(s4, atlas::math::Vector<float, 3>(2.0f, 4.0f, 6.0f), eps));

    const atlas::math::Vector<float, 3> s5 = b / 10.0f;
    EXPECT_TRUE(atlas::test::vec_near(s5, atlas::math::Vector<float, 3>(1.0f, 2.0f, 3.0f), eps));

    const atlas::math::Vector<float, 3> s6 = atlas::math::cmin(a, b);
    EXPECT_TRUE(atlas::test::vec_near(s6, atlas::math::Vector<float, 3>(1.0f, 2.0f, 3.0f), eps));

    const atlas::math::Vector<float, 3> s7 = atlas::math::cmax(a, b);
    EXPECT_TRUE(atlas::test::vec_near(s7, atlas::math::Vector<float, 3>(10.0f, 20.0f, 30.0f), eps));
}
