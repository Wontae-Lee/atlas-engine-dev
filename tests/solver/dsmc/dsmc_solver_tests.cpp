...
class ExposedDsmcSolver final : public atlas::DsmcSolver {
public:
    using atlas::DsmcSolver::DsmcSolver;
...
TEST(DsmcSolver, EnsureStatesCreatesAllDsmcUniverseStates) {
    ...
    const auto expected_size = static_cast<std::size_t>(universe->cell_count());
    ASSERT_NE(universe->state<atlas::UniverseNumberParticleState>(), nullptr);
    ASSERT_NE(universe->state<atlas::UniverseMaxRelativeSpeedState>(), nullptr);
    ASSERT_NE(universe->state<atlas::UniverseMaxSigmaGState>(), nullptr);
    ASSERT_NE(universe->state<atlas::UniverseCollisionRemainderState>(), nullptr);
    ASSERT_NE(universe->state<atlas::UniverseCollisionCountState>(), nullptr);
    EXPECT_EQ(universe->state<atlas::UniverseNumberParticleState>()->data().size(), expected_size);
    EXPECT_EQ(universe->state<atlas::UniverseCollisionCountState>()->data().size(), expected_size);
}
...
TEST(DsmcSolver, ResetStatesClearsCollisionStateBuffers) {
    ...
    universe->state<atlas::UniverseNumberParticleState>()->data()[0] = 2.0f;
    universe->state<atlas::UniverseMaxRelativeSpeedState>()->data()[0] = 3.0f;
    universe->state<atlas::UniverseMaxSigmaGState>()->data()[0] = 4.0f;
    universe->state<atlas::UniverseCollisionRemainderState>()->data()[0] = 0.5f;
    universe->state<atlas::UniverseCollisionCountState>()->data()[0] = 6;

    solver.reset_states();

    EXPECT_EQ(universe->state<atlas::UniverseNumberParticleState>()->data()[0], 0.0f);
    EXPECT_EQ(universe->state<atlas::UniverseMaxRelativeSpeedState>()->data()[0], 0.0f);
    EXPECT_EQ(universe->state<atlas::UniverseMaxSigmaGState>()->data()[0], 0.0f);
    EXPECT_EQ(universe->state<atlas::UniverseCollisionRemainderState>()->data()[0], 0.0f);
    EXPECT_EQ(universe->state<atlas::UniverseCollisionCountState>()->data()[0], 0);
}
...
TEST(DsmcSolver, MakeProbeCapturesFluidUniverseAndSearcherViews) {
    ...
    EXPECT_EQ(probe.cell_count, universe->cell_count());
...
TEST(DsmcSolver, ParticleAtMapsLocalCellIndexToParticleIndex) {
    ...
    EXPECT_EQ(atlas::DsmcSolver::particle_at(1, 0, 3, 4, indices), 1);
    EXPECT_EQ(atlas::DsmcSolver::particle_at(-1, 0, 3, 4, indices), -1);
    EXPECT_EQ(atlas::DsmcSolver::particle_at(3, 0, 3, 4, indices), -1);
    EXPECT_EQ(atlas::DsmcSolver::particle_at(0, 0, 3, 2, indices), -1);
}
...
TEST(DsmcSolver, CollidePairRejectsInvalidParticleIndices) {
    ...
    EXPECT_FALSE(atlas::DsmcSolver::collide_pair(
        probe,
        0,
        0,
        0,
