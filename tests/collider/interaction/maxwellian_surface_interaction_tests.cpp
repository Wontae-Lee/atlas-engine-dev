...
TEST(MaxwellianSurfaceInteraction, DefaultStateIsWellDefined) {
    const MaxwellianSurfaceInteraction<float> interaction;
...
TEST(MaxwellianSurfaceInteraction, SettersUpdateState) {
    MaxwellianSurfaceInteraction<float> interaction;
...
TEST(MaxwellianSurfaceInteraction, MostProbableSpeedMatchesKineticTheoryScale) {
    ...
    const float expected = std::sqrt(2.0f * static_cast<float>(boltzmann_constant) * 300.0f / 2.0f);
...
TEST(MaxwellianSurfaceInteraction, BuilderRejectsInvalidAccommodationCoefficients) {
    EXPECT_THROW(
        MaxwellianSurfaceInteraction<float>::builder()
...
TEST(MaxwellianSurfaceInteraction, MomentumAccommodationZeroUsesSpecularReflection) {
    ...
    const Vector3F out = interaction.sample(
        incident,
        normal,
        1.0f,
        0.5f,
    ...
    EXPECT_TRUE(vec_near(out, reflected(incident, normal), tol));
}
...
TEST(MaxwellianSurfaceInteraction, InternalEnergyAccommodationOneSamplesWallEnergy) {
    ...
    const float wall_energy = static_cast<float>(boltzmann_constant) * 300.0f;
...
TEST(MaxwellianSurfaceInteraction, InternalEnergyUsesDeterministicSamples) {
    ...
    const auto out = interaction.internal_energy(FluidInternalEnergy<float> { 1.0f, 2.0f, 3.0f });
...
TEST(MaxwellianSurfaceInteraction, DiffuseSurfaceHitClearsDisabledInternalEnergyModes) {
    ...
    const auto out = interaction.internal_energy(
        FluidInternalEnergy<float> { 1.0f, 2.0f, 3.0f },
        Vector3F(1.0f, 0.0f, 0.0f),
        Vector3F(1.0f, 0.0f, 0.0f),
        material);
...
TEST(MaxwellianSurfaceInteraction, DiffuseRotationalEnergySamplerHandlesDiscreteAndDisabledModes) {
    const auto discrete = MaxwellianSurfaceInteraction<float>::builder()
                              .with_rot_style(MaxwellianInternalEnergyStyle::discrete)
                              .build();
    const auto disabled = MaxwellianSurfaceInteraction<float>::builder()
    ...
    const float discrete_energy = discrete.sample_diffuse_rotational_energy(material, Vector3F(1.0f, 2.0f, 3.0f));
    const float disabled_energy = disabled.sample_diffuse_rotational_energy(material, Vector3F(1.0f, 2.0f, 3.0f));
...
TEST(MaxwellianSurfaceInteraction, DiffuseVibrationalEnergySamplerHandlesDiscreteAndDisabledModes) {
    ...
    const float discrete_energy = discrete.sample_diffuse_vibrational_energy(material, Vector3F(1.0f, 2.0f, 3.0f));
    const float disabled_energy = disabled.sample_diffuse_vibrational_energy(material, Vector3F(1.0f, 2.0f, 3.0f));
