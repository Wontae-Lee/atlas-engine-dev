...
template <typename T>
void
ColliderCollisionKernel<T>::resolve_particles(const ColliderProbe<T>& probe,
                                              const PostColliderType post_collider_type,
                                              const T dt) {
    const auto collision_probe = probe;
    const PostColliderKernel<T> post_collider_kernel(post_collider_type);

    atlas::parallel_for<ExecutionPolicy::device>(
        0,
        collision_probe.particle_count,
        [=] ATLAS_DEVICE(const int i) {
            const ParticleSweep sweep = ColliderCollisionKernel<T>::make_particle_sweep(
                collision_probe,
                i,
                dt);
            ...
            const ColliderHit<T> hit = ColliderCollisionKernel<T>::closest_hit(
                collision_probe,
            ...
            ColliderCollisionKernel<T>::resolve_hit(
                collision_probe,
                post_collider_kernel,
                i,
                sweep,
...
template <typename T>
ATLAS_DEVICE typename ColliderCollisionKernel<T>::ParticleSweep
ColliderCollisionKernel<T>::make_particle_sweep(const ColliderProbe<T>& probe,
                                                const int particle_index,
                                                const T dt) {
    const Vector3<T> velocity  = probe.velocities[particle_index];
...
template <typename T>
ATLAS_DEVICE ColliderHit<T>
ColliderCollisionKernel<T>::closest_hit(const ColliderProbe<T>& probe,
                                        const PostColliderKernel<T>& post_collider_kernel,
                                        const ParticleSweep& sweep,
                                        const T dt) {
    ColliderHit<T> hit {};
    T closest_time = atlas::far;

    const bool moving_surface_sweep = post_collider_kernel.type == PostColliderType::precise;
    const atlas::Ray<T> particle_ray(sweep.origin, sweep.direction);
...
template <typename T>
ATLAS_DEVICE void
ColliderCollisionKernel<T>::resolve_hit(const ColliderProbe<T>& probe,
                                        const PostColliderKernel<T>& post_collider_kernel,
                                        const int particle_index,
                                        const ParticleSweep& sweep,
