...
MaxwellianSurfaceInteraction<T>::operator()(const Vector3<T>& incident,
    ...
    const T branch_sample = atlas::sample_hashed_unit_interval(
        incident + normal * T(atlas::RANDOM_HASH_NORMAL_SCALE_FOR_MIX),
        T(atlas::RANDOM_HASH_SALT_MIX));

    const T perpendicular_sample = atlas::sample_hashed_unit_interval(
        incident,
        T(atlas::RANDOM_HASH_SALT_DIFFUSE_U1));

    const T theta_sample = atlas::sample_hashed_unit_interval(
        normal + incident,
        T(atlas::RANDOM_HASH_SALT_DIFFUSE_U2));

    const T tangent_sample = atlas::sample_hashed_unit_interval(
        incident + normal * T(3.0),
        T(4.19));

    const Vector3<T> tangent_seed(
        atlas::sample_hashed_unit_interval(incident, T(5.11)),
        atlas::sample_hashed_unit_interval(normal, T(6.23)),
        atlas::sample_hashed_unit_interval(incident + normal, T(7.37)));
...
MaxwellianSurfaceInteraction<T>::sample(const Vector3<T>& incident,
    ...
    Vector3<T> tangent1 = atlas::reject(incident, normal);
    ...
    const Vector3<T> tangent2 = atlas::cross(normal, tangent1);
...
MaxwellianSurfaceInteraction<T>::internal_energy(
    const FluidInternalEnergy<T>& incident) const noexcept {
    const Vector3<T> seed(incident.translational, incident.rotational, incident.vibrational);

    return sample_internal_energy(
        incident,
        atlas::sample_hashed_unit_interval(seed, T(8.11)),
        atlas::sample_hashed_unit_interval(seed, T(9.23)),
        atlas::sample_hashed_unit_interval(seed, T(10.37)),
        atlas::sample_hashed_unit_interval(seed, T(11.41)),
        atlas::sample_hashed_unit_interval(seed, T(12.53)),
        atlas::sample_hashed_unit_interval(seed, T(13.67)));
}
...
MaxwellianSurfaceInteraction<T>::internal_energy(
    ...
    const Vector3<T>& normal) const noexcept {
    const T branch_sample = atlas::sample_hashed_unit_interval(
        incident_velocity + normal * T(atlas::RANDOM_HASH_NORMAL_SCALE_FOR_MIX),
        T(atlas::RANDOM_HASH_SALT_MIX));
...
MaxwellianSurfaceInteraction<T>::sample_internal_energy(
    const FluidInternalEnergy<T>& incident,
...
MaxwellianSurfaceInteraction<T>::sample_internal_energy_mode(const T incident,
    ...
    const T magnitude   = atlas::sqrt_nonnegative(safe_energy * (T(1) - acc) / safe_wall);
    const T radius      = atlas::sqrt_nonnegative(-acc * std::log(detail::maxwellian_unit_sample(sample)));
...
MaxwellianSurfaceInteraction<T>::Builder::validate() const {
    ...
    if (!atlas::isfinite(_molecular_mass) || _molecular_mass <= T(0)) {
        throw std::runtime_error(
