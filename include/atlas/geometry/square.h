...
public:
    ...
    closest_point(const Vector3& p) const noexcept {
        ...
        const Vector3 center_to_point = p - center;

        const float signed_plane_offset = center_to_point.dot(unit_normal);
        const Vector3 projected_point   = p - unit_normal * signed_plane_offset;

        const Vector3 planar_offset = projected_point - center;
    ...
    signed_distance(const Vector3& p) const noexcept {
        const Vector3 projected_closest_point = closest_point(p);

        const Vector3 unit_normal = closest_normal(p);
    ...
    bound() const noexcept {
        ...
        const Vector3 extent = (atlas::abs(tangent) + atlas::abs(bitangent)) * half_side;
    ...
    trace(const Ray& ray) const noexcept {
        ...
        const Vector3 hit_point = ray.point_at(distance);

        const Vector3 center_to_hit = hit_point - center;
...
private:
    ...
    build_basis(const Vector3& input_normal,
                Vector3& unit_normal,
                Vector3& tangent,
                Vector3& bitangent) const noexcept {
...
public:
    ...
    ATLAS_HOST ATLAS_NODISCARD Square
    build() const;

    ATLAS_HOST ATLAS_NODISCARD atlas::host_shared_ptr<Square>
