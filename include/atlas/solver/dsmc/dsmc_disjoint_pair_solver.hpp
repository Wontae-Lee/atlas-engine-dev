...
template <typename T>
void
DsmcDisjointPairSolver<T>::apply_collisions(
    const typename DsmcSolver<T>::DsmcSolverProbe& probe,
    const int index,
    const T) {
    ...
    atlas::parallel_for<ExecutionPolicy::device>(
        ...
        [=] ATLAS_DEVICE(const int cell) {
            ...
            // Limit candidate collisions to disjoint local pairs: (0,1), (2,3), ...
            ...
            // No collision work is possible without at least one pair and one requested collision.
            ...
            // Process no more than the requested collision count or the available disjoint pairs.
            ...
            for (int local_collision = 0; local_collision < collision_limit; ++local_collision) {
                // Map collision id to local disjoint pair indices.
...
template <typename T>
typename DsmcDisjointPairSolver<T>::Builder&
DsmcDisjointPairSolver<T>::Builder::with_searcher(
...
template <typename T>
typename DsmcDisjointPairSolver<T>::Builder&
DsmcDisjointPairSolver<T>::Builder::with_kernel_type(
    const DsmcKernelType kernel_type) noexcept {
