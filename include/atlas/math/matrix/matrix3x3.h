...
public:
    ...
    ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE constexpr Matrix3x3() noexcept
        : m00(0.0f)
        , m01(0.0f)
        , m02(0.0f)
        , m10(0.0f)
    ...
    ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE explicit constexpr Matrix3x3(const float s) noexcept
        : m00(s)
        , m01(0.0f)
        , m02(0.0f)
        , m10(0.0f)
    ...
    ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE constexpr Matrix3x3(const float a00, const float a01, const float a02,
    ...
    ATLAS_HOST ATLAS_FORCE_INLINE explicit Matrix3x3(std::initializer_list<float> list) noexcept {
    ...
    Matrix3x3&
    operator=(const Matrix3x3&) noexcept = default;
    ...
    ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE void
    add(const Matrix3x3& m) noexcept {
        for (int i = 0; i < 9; ++i) _data[i] += m._data[i];
    }

    ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE void
    sub(const Matrix3x3& m) noexcept {
        for (int i = 0; i < 9; ++i) _data[i] -= m._data[i];
    }

    ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE Matrix3x3&
    ...
    ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE Matrix3x3&
    operator+=(const Matrix3x3& m) noexcept {
        add(m);
        return *this;
    }
    ...
    operator==(const Matrix3x3& o) const noexcept {
        for (int i = 0; i < 9; ++i) {
    ...
    try_inverse(Matrix3x3& out, const float eps = std::numeric_limits<float>::epsilon()) const noexcept {
    ...
    ATLAS_NODISCARD ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE Matrix3x3
    mul(const Matrix3x3& r) const noexcept {
    ...
    mul(const Vector3& v) const noexcept {
        return Vector3(std::fma(m01, v.y, std::fma(m02, v.z, m00 * v.x)),
                       std::fma(m11, v.y, std::fma(m12, v.z, m10 * v.x)),
                       std::fma(m21, v.y, std::fma(m22, v.z, m20 * v.x)));
    }

    ATLAS_NODISCARD ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE Vector3
    solved(const Vector3& b) const noexcept {
        return inversed().mul(b);
    }

    ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE bool
    solve(const Vector3& b, Vector3& x,
...
private:
    ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE void
    cofactor_inverse(Matrix3x3& out, const float det) const noexcept {
        ...
        out.set(c00 * inv_det, c10 * inv_det, c20 * inv_det,
...
ATLAS_NODISCARD ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE Matrix3x3
transpose(const Matrix3x3& m) noexcept {
    return m.transposed();
}

ATLAS_NODISCARD ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE float
determinant(const Matrix3x3& m) noexcept {
    return m.determinant();
}

ATLAS_NODISCARD ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE Matrix3x3
inverse(const Matrix3x3& m) noexcept {
    return m.inversed();
}
...
ATLAS_NODISCARD ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE Matrix3x3
operator*(const Matrix3x3& a, const float s) noexcept {
    Matrix3x3 out;
...
ATLAS_NODISCARD ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE Matrix3x3
operator*(const float s, const Matrix3x3& a) noexcept {
    return a * s;
}
...
ATLAS_NODISCARD ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE Vector3
operator*(const Matrix3x3& a, const Vector3& v) noexcept {
    return a.mul(v);
}

ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE void
rotate(const Matrix3x3& matrix, const Vector3& input, Vector3& output) noexcept {
...
ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE void
rotate_translate(const Matrix3x3& matrix,
                 const Vector3& input,
...
ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE void
rotate_subtract(const Matrix3x3& matrix,
                const Vector3& input,
                const Vector3& offset,
                Vector3& output) noexcept {
...
ATLAS_NODISCARD ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE bool
solve(const Matrix3x3& A, const Vector3& b, Vector3& x,
      const float eps = std::numeric_limits<float>::epsilon()) noexcept {
    return A.solve(b, x, eps);
}

ATLAS_NODISCARD ATLAS_ALL_DEVICE ATLAS_FORCE_INLINE Vector3
solve(const Matrix3x3& A, const Vector3& b) noexcept {
    return A.solved(b);
}
