...
Vector<T, 3>::tangential() const noexcept {
    ...
    Vector t1;
    if (ax > ay) {
        const T inv = T(1) / static_cast<T>(std::sqrt(x * x + z * z + (x * x + z * z == 0 ? 1 : 0)));
        t1          = Vector(-z * inv, T(0), x * inv);
    } else {
        const T inv = T(1) / static_cast<T>(std::sqrt(y * y + z * z + (y * y + z * z == 0 ? 1 : 0)));
...
Vector<T, 3>::cast_to() const noexcept {
    return Vector<To, 3>(static_cast<To>(x), static_cast<To>(y), static_cast<To>(z));
}
...
tangential(const Vector<T, 3>& normal) noexcept {
    ...
    Vector<T, 3> t1;
    if (ax > ay) {
        const T inv = T(1) / static_cast<T>(std::sqrt(nx * nx + nz * nz + (nx * nx + nz * nz == 0 ? 1 : 0)));
        t1          = Vector<T, 3>(-nz * inv, T(0), nx * inv);
    } else {
        const T inv = T(1) / static_cast<T>(std::sqrt(ny * ny + nz * nz + (ny * ny + nz * nz == 0 ? 1 : 0)));
...
ceil(const Vector3<T>& a) {
    return Vector3<T>(std::ceil(a.x), std::ceil(a.y), std::ceil(a.z));
}
