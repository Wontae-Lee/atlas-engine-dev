...
/**
 * @brief Charged-species leaf of the `Material` tagged union.
 *
 * `Ion` mirrors `Molecule` exactly in layout and contract (see `molecule.h`):
 * eight trivially copyable `float`s that satisfy `ConceptMaterial` and copy
 * safely into device memory. It exists as its own type so charged-species
 * physics can diverge later. The charge itself is *not* modelled by this
 * struct — no electromagnetic field coupling is stored here — consistent with
 * the engine's preference for simple, large-domain simulation over fine
 * physical detail.
 *
 ...
 *
 * @see ConceptMaterial, Material, MaterialType::ion, Molecule
 */
class Ion final {
public:
    /**
     * @brief Constructs an ion with every property zero-initialized.
     *
    ...
    /**
     * @brief Constructs an ion from its full set of physical constants.
     *
