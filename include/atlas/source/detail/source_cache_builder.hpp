...
template <typename T>
void
SourceCacheBuilder<T>::rebuild(const DeviceBuffer<Unit<T>>& units,
    ...
    const HostBuffer<Unit<T>> host_units(units.begin(), units.end());
    const HostBuffer<SpawnOperator<T>> host_spawn_operators(
        spawn_operators.begin(),
        spawn_operators.end());
#else
    const auto& host_units           = units;
    const auto& host_spawn_operators = spawn_operators;
#endif

    local_unit_counts.clear();
    local_unit_counts.resize(host_units.size(), 0);

    HostBuffer<Vector3<T>> host_flat_positions;
    ...
    for (std::size_t unit_index = 0; unit_index < host_units.size(); ++unit_index) {
        const auto& geometry = host_units[unit_index].geometry_operator();
...
template <typename T>
void
SourceCacheBuilder<T>::clear(HostBuffer<int>& local_unit_counts,
                             DeviceBuffer<Vector3<T>>& flat_local_positions,
                             DeviceBuffer<int>& flat_unit_indices,
                             std::size_t& local_particle_count,
                             DeviceBuffer<std::size_t>& species_cache,
