...
Both are configured with one or more `Unit<T>` regions (geometry plus optional
kinematics) using `.with_units(...)`.
...
motion. It owns boundary `Unit<T>` regions and per-boundary surface-interaction
kernels (for example isothermal or Maxwellian wall models), plus a
post-collision motion policy.
