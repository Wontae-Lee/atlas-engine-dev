...
simulation engine, with a focus on the public headers under `include/atlas/`
and their compiled definitions under `src/atlas/`. It is written for
contributors (human or AI) who need to understand how the modules fit together
before changing code.

Atlas is a C++20 particle simulation engine compiled entirely with nvcc, using
a single `float` scalar type, with the TBB or CUDA backend selected through
the Thrust device system, optional serialization, and optional nanobind-based
Python bindings. The public API is aggregated through a single generated
umbrella header:
