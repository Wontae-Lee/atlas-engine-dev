...
| [`release.yml`](../../.github/workflows/release.yml) | Manual dispatch from `main` with `X.Y.Z` input and an existing matching tag | Version verification, reusable package and image builds, PyPI upload, and GitHub Release creation | PyPI distributions, GHCR images, and a GitHub Release |
...
commit, verifies strict `X.Y.Z` input, matching CMake, Python, citation, and
README versions, and confirms that the existing tag points to that commit. It
reuses the Python workflow to build distributions, uploads them to PyPI from
`release.yml`, and calls the Docker workflow with publishing enabled. It creates
a GitHub Release only after
