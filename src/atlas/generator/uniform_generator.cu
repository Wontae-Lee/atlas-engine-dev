...
UniformGenerator::generate(FluidVelocityState* velocities,
    ...
    // Every written index must be valid in both buffers, so the usable capacity
    // is the smaller of the two; a shorter buffer bounds the whole write.
    ...
    atlas::parallel_for<ExecutionPolicy::device>(
        ...
        [=] ATLAS_ALL_DEVICE(const std::size_t i) {
            ...
            // Fold the absolute slot index with the seed so each particle gets a
            // decorrelated, reproducible RNG stream regardless of launch order.
            ...
            // Weighted species pick, then a uniform velocity shifted by the drift.
