...
StateBridge::upload_raw(const void* source,
    ...
    if (inserted) {
        const cudaError_t result = cudaGraphicsGLRegisterBuffer(&entry->second,
                                                                 target.id(),
                                                                 cudaGraphicsRegisterFlagsWriteDiscard);
        if (result != cudaSuccess) {
            _impl->resources.erase(entry);
            check_cuda(result, "CUDA/OpenGL buffer registration failed");
        }
    }
    ...
    if (pointer_result != cudaSuccess || mapped == nullptr || mapped_bytes < bytes) {
        ...
        throw std::runtime_error("CUDA/OpenGL mapped buffer is smaller than the requested upload.");
    }
