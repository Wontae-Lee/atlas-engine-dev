...
void
register_serialization(nb::module_& m) {
    m.def("save_fluid_binary", [](const Fluid& fluid, const std::filesystem::path& path) {
        save_fluid_binary(fluid, path.string());
    }, "fluid"_a, "path"_a);

    m.def("save_fluid_binary", [](const PySystem& system, const std::filesystem::path& path) {
        save_fluid_binary(*system.value.fluid(), path.string());
    }, "system"_a, "path"_a);

    m.def("save_universe_binary", [](const Universe& universe, const std::filesystem::path& path) {
        save_universe_binary(universe, path.string());
    }, "universe"_a, "path"_a);

    m.def("save_universe_binary", [](const PySystem& system, const std::filesystem::path& path) {
        save_universe_binary(*system.value.universe(), path.string());
    }, "system"_a, "path"_a);

    m.def("load_fluid_binary", [](const std::filesystem::path& path) {
    ...
    m.def("load_universe_binary", [](const std::filesystem::path& path) {
    ...
    m.def("restore_fluid", [](const std::filesystem::path& path) {
        return restore_fluid(path.string());
    }, "path"_a, "Restore a device-resident Fluid from a binary snapshot.");

    m.def("restore_universe", [](const std::filesystem::path& path) {
        return restore_universe(path.string());
    }, "path"_a, "Restore a device-resident Universe from a binary snapshot.");
}
